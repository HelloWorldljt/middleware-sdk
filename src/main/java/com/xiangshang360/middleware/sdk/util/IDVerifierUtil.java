...
public class IDVerifierUtil {
	private static final Log LOG = LogFactory.getLog(IDVerifierUtil.class);
	 ...
	 * @date 2019年9月19日
	 */
	public  boolean isAdultAged() {
		Date birthdayDate = this.getBirthday();
		Date now=CalendarUtil.getCurrentDate();
		int age=CalendarUtil.getBetweenYearNumber(birthdayDate, now);
		if(age>=75){
			return false;
		}
		return true;
	...
	private boolean isValidate15Idcard(String idcard) {
		...
		if (isDigital(idcard)) {
			...
			try {
				...
				LOG.error(LogUtils.template("isValidate15Idcard", null, e.getMessage(), null),e);
			}
	...
	private String convertIdcarBy15bit(String idcard) {
		...
		if (isDigital(idcard)) {
			...
			try {
				...
				LOG.error(LogUtils.template("convertIdcarBy15bit", null, e.getMessage(), null),e);
			}
	...
	public IDVerifierUtil initIdProperties(String idcard) {
		try {
			...
			LOG.error(LogUtils.template("initIdProperties", null, e.getMessage(), null),e);
		}
		return this;
	}
